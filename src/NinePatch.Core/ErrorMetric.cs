...
public static class ErrorMetric
{
    ...
    /// <summary>
    /// Check sRGB error for a single row's [colB..colE) region. Early-exits on first exceeding pixel.
    /// Used by Search1D.TryN (axis=1) for row-level early exit.
    /// </summary>
    internal static bool PassesThresholdSliceX(
        PrecomputedSrgb origSrgb, SoaImage reconstructed,
        int row, int colB, int colE, int imgWidth, float threshold, bool alphaWeighted = true)
    {
        int vecLen = Vector<float>.Count;
        var vZero = Vector<float>.Zero;
        var vThreshold = new Vector<float>(threshold);
        int rowOffset = row * imgWidth;

        // SIMD pass over [colB..colE)
        int start = rowOffset + colB;
        int end = rowOffset + colE;
        int alignedEnd = start + ((end - start) / vecLen) * vecLen;

        for (int i = start; i < alignedEnd; i += vecLen)
        {
            var oA = new Vector<float>(origSrgb.Alpha, i);
            var rA = new Vector<float>(reconstructed.A, i);
            var alphaWeight = alphaWeighted ? Vector.Max(oA, rA) : Vector<float>.One;
        ...
        {
            float rErr = MathF.Abs(origSrgb.R[i] - ColorSpace.LinearToSrgbFloat(reconstructed.R[i])) * 255f;
            float gErr = MathF.Abs(origSrgb.G[i] - ColorSpace.LinearToSrgbFloat(reconstructed.G[i])) * 255f;
            float bErr = MathF.Abs(origSrgb.B[i] - ColorSpace.LinearToSrgbFloat(reconstructed.B[i])) * 255f;
            float aErr = MathF.Abs(origSrgb.Alpha[i] - reconstructed.A[i]) * 255f;
            if (alphaWeighted)
            {
                float w = MathF.Max(origSrgb.Alpha[i], reconstructed.A[i]);
    ...
    /// <summary>
    /// Check sRGB error for a column block [colBlock..colBlock+colBlockLen) across rows [rowB..rowE).
    /// Used by Search1D.TryN (axis=0) for column-block-level early exit.
    /// </summary>
    internal static bool PassesThresholdSliceY(
        PrecomputedSrgb origSrgb, SoaImage reconstructed,
        int rowB, int rowE, int colBlock, int colBlockLen, int imgWidth, float threshold, bool alphaWeighted = true)
    {
        int vecLen = Vector<float>.Count;
        var vZero = Vector<float>.Zero;

        for (int y = rowB; y < rowE; y++)
        {
            int start = y * imgWidth + colBlock;
            int end = start + colBlockLen;
            int alignedEnd = start + ((end - start) / vecLen) * vecLen;
