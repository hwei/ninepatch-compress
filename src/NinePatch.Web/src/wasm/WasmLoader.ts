...
interface WasmInstance {
  NinePatch: {
    Wasm: {
      WasmExports: {
        Compress: (rgba: Uint8Array, w: number, h: number, threshold: number, margin: number, minSavings: number) => string;
        GetVersion: () => string;
      };
    };
  };
...
export async function compress(
  ...
  const json = wasmExports!.NinePatch.Wasm.WasmExports.Compress(rgba, width, height, threshold, margin, minSavings);
